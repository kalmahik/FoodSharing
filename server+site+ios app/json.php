...
for ($i=$min[0]; $i <= $max[0]; $i++) { 
	...
	$curid = mysql_fetch_array(mysql_query("SELECT pk_i_id FROM oc_t_item WHERE fk_i_category_id=$cat", $db));
	//echo $curid[0];
	$title = mysql_fetch_array(mysql_query("SELECT s_title FROM oc_t_item_description WHERE fk_i_item_id=$curid[$x]", $db));
	$desc = mysql_fetch_array(mysql_query("SELECT s_description FROM oc_t_item_description WHERE fk_i_item_id=$curid[$x]", $db));
	$cat_id = mysql_fetch_array(mysql_query("SELECT fk_i_category_id FROM oc_t_item WHERE fk_i_category_id=$cat", $db));
	$date = mysql_fetch_array(mysql_query("SELECT dt_pub_date FROM oc_t_item WHERE pk_i_id=$curid[$x] AND fk_i_category_id=$cat", $db));
	$img = mysql_fetch_array(mysql_query("SELECT s_name FROM oc_t_item_resource WHERE fk_i_item_id=$curid[$x]", $db));
	$limg = mysql_fetch_array(mysql_query("SELECT pk_i_id FROM oc_t_item_resource WHERE fk_i_item_id=$curid[$x]", $db));
	$ext = mysql_fetch_array(mysql_query("SELECT s_extension FROM oc_t_item_resource WHERE fk_i_item_id=$curid[$x]", $db));
...
if($_GET['id']){
	$i = $min[0] = $max[0] = strip_tags($_GET['id']);
	$title = mysql_fetch_array(mysql_query("SELECT s_title FROM oc_t_item_description WHERE fk_i_item_id=$i", $db));
	$desc = mysql_fetch_array(mysql_query("SELECT s_description FROM oc_t_item_description WHERE fk_i_item_id=$i", $db));
	$cat_id = mysql_fetch_array(mysql_query("SELECT fk_i_category_id FROM oc_t_item WHERE pk_i_id=$i", $db));
	$date = mysql_fetch_array(mysql_query("SELECT dt_pub_date FROM oc_t_item WHERE pk_i_id=$i", $db));
...
echo str_replace("\/", '/', json_encode($mas, JSON_UNESCAPED_UNICODE));
